...
class Paks extends StatelessWidget {
  ...
  void addUserText(String text, String desc) async {
    ...
    List<PakDataItem> d = <PakDataItem>[];
    d.add(new PakDataItem(text, desc));
    d.add(new PakDataItem("foo", "bar"));
    PakData data = new PakData(d, "TEST");
  ...
  @deprecated
  Future<PakData> readFromDatabase() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String uid = prefs.getString('user_id')!;
    // get pak name as collection
    ...
    final PakData pakData = PakData(pakDataItemsList, "");
